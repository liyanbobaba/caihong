...
function msg(tip,t) {t=t || 1500;top.layer.msg(tip,{opacity:0.4,time:t});}
function error(tip,t){t=t || 1500;top.layer.alert(tip,{opacity:0.4,time:t,icon:2});}
 //将时间减去1秒，计算天、时、分、秒
...
function computeNum(obj, isPlus) {
	...
	if(n > max) {
		...
		if(type==1){
			...
			msg(txtNum.data("msg")? txtNum.data("msg"):"抱歉，此商品单次限购" + n + "件");
		}
